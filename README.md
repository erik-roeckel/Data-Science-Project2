...
### Description
This is the **second assignment** for the CS 1656 -- Introduction to Data Science (CS 2056) class, for the Spring 2020 semester.
...
* update a skeleton Python script (`pmovies.py`) in order to read input from CSV files and insert the data into the `cs1656.sqlite` database, and   
...
The schema of the database is embedded in the `pmovies.py` Python script and should not be modified. It is as follows:
...
All the data should be inserted into the appropriate tables into the `cs1656.sqlite` database. Sample insert statements have been provided in the `pmovies.py` script, but you are not restricted to doing the insertions in exactly the same way.
...
xml
```
If you would like to use any other libraries, you must ask permission by Friday, Feb 8, 2020, using [canvas](http://cs1656.org).

---

### How to submit your assignment
For this assignment, you must use the repository that was created for you after visiting the classroom link. You need to update the  file `pmovies.py` as described above, and add other files that are needed for running your program. You need to make sure to commit your code to the repository provided.

The due date is **Monday, Feb 17, 2020 (23:59pm)**  
